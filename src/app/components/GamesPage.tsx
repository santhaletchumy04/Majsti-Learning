...
import { GrammarBuilderGame } from './games/GrammarBuilderGame';
import { StoryCreatorGame } from './games/StoryCreatorGame';
import { Gamepad2, Trophy, Star } from 'lucide-react';
import type { Language } from '../App';

type GameType = 'menu' | 'wordMatch' | 'quiz' | 'memory' | 'wordSearch' | 'speedTyping' | 'grammarBuilder' | 'storyCreator';
...
export function GamesPage({ language }: GamesPageProps) {
  ...
  if (currentGame === 'speedTyping') {
    return <SpeedTypingGame onBack={() => setCurrentGame('menu')} onScoreUpdate={(score) => setTotalScore(prev => prev + score)} />;
  }

  if (currentGame === 'grammarBuilder') {
    return <GrammarBuilderGame onBack={() => setCurrentGame('menu')} onScoreUpdate={(score) => setTotalScore(prev => prev + score)} />;
  }

  if (currentGame === 'storyCreator') {
    return <StoryCreatorGame onBack={() => setCurrentGame('menu')} onScoreUpdate={(score) => setTotalScore(prev => prev + score)} />;
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <GameCard
          ...
          points="25-125"
          color="from-accent/20 to-accent/5"
          onClick={() => setCurrentGame('grammarBuilder')}
        />
        ...
        <GameCard
          ...
          points="20-100"
          color="from-secondary/30 to-secondary/10"
          onClick={() => setCurrentGame('storyCreator')}
        />
      </div>
